...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { min-height: 100vh; display: flex; justify-content: center; align-items: center; background: #0a0e17; font-family: 'Rajdhani', sans-serif; padding: 16px; }
        .card { max-width: 480px; width: 100%; background: rgba(10,14,23,0.75); backdrop-filter: blur(24px); border-radius: 40px; padding: 32px 24px; border: 1px solid rgba(255,255,255,0.06); }
        .logo { font-size: 28px; font-weight: 900; text-align: center; background: linear-gradient(135deg, #ff0040, #ff6b00); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        .subtitle { text-align: center; color: #00f0ff; font-size: 14px; letter-spacing: 3px; text-transform: uppercase; }
        .input-field { width: 100%; padding: 14px 18px; border-radius: 16px; border: 1px solid rgba(0,255,255,0.12); background: rgba(0,0,0,0.5); color: #fff; font-size: 16px; outline: none; }
        .btn { width: 100%; padding: 16px; border: none; border-radius: 60px; font-family: 'Orbitron', monospace; font-weight: 700; font-size: 15px; cursor: pointer; }
        .btn-fire { background: linear-gradient(135deg, #ff0040, #ff6b00); color: #fff; }
        .btn-stop { background: rgba(255,50,50,0.15); color: #ff6a6a; border: 1px solid rgba(255,50,50,0.3); }
        .flex { display: flex; gap: 12px; }
        .result-box { margin-top: 20px; padding: 16px; border-radius: 20px; background: rgba(0,255,0,0.04); color: #90ff90; text-align: center; }
        .result-box.error { background: rgba(255,0,0,0.06); color: #ff7a7a; }
        .footer { text-align: center; margin-top: 24px; color: #3d4a66; }
    </style>
</head>
<body>
<div class="card">
    ...
    <form method="POST" id="bombForm">
        ...
        <input type="number" name="delay" class="input-field" value="2" step="0.5" min="0.5" placeholder="Delay (sec)">
        <div class="flex">
    ...
    {% if result %}
        <div class="result-box {% if '❌' in result or '⚠️' in result %}error{% endif %}">{{ result }}</div>
    {% endif %}
    <div class="footer">Developed by Arif</div>
</div>
<script>
    function stopBomb() {
        ...
        .then(r => r.json())
        .then(data => {
            if (data.status === 'stopped') { alert('Stopped'); location.reload(); }
            else alert('No active bombing');
        })
